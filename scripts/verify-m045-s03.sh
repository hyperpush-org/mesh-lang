...
if not new_names:
    raise SystemExit('expected fresh .tmp/m045-s03 artifact directories from the S03 e2e replay')
...
if not scenario_dir.name.startswith('scaffold-failover-runtime-truth-'):
    raise SystemExit(
        f'{scenario_dir}: expected scaffold failover runtime truth artifact prefix'
    )
...
prekill_search = scenario_dir / 'pre-kill-search'
if not prekill_search.is_dir():
    raise SystemExit(f'{scenario_dir}: missing pre-kill-search retained directory')
if not (prekill_search / 'selected.json').is_file():
    raise SystemExit(f'{prekill_search}: missing selected.json')
...
run_expect_success m044-s04-replay 00-m044-s04-replay no 3600 \
  bash scripts/verify-m044-s04.sh
run_expect_success tooling-clustered-init 01-tooling-clustered-init yes 1200 \
  cargo test -p meshc --test tooling_e2e test_init_clustered_creates_project -- --nocapture
run_expect_success m045-s02-runtime-completion 02-m045-s02-runtime-completion yes 2400 \
  cargo test -p meshc --test e2e_m045_s02 m045_s02_scaffold_runtime_completion_ -- --nocapture

S03_BEFORE="$ARTIFACT_DIR/03-m045-s03.before.txt"
capture_m045_s03_snapshot "$S03_BEFORE"
run_expect_success m045-s03-e2e 03-m045-s03-e2e yes 3600 \
  cargo test -p meshc --test e2e_m045_s03 m045_s03_failover_ -- --nocapture
record_phase m045-s03-artifacts started
BUNDLE_ROOT="$ARTIFACT_DIR/retained-m045-s03-artifacts"
copy_new_m045_s03_artifacts \
  m045-s03-artifacts \
  "$S03_BEFORE" \
  "$BUNDLE_ROOT" \
  "$ARTIFACT_DIR/03-m045-s03-artifacts.txt"
printf '%s\n' "$BUNDLE_ROOT" >"$LATEST_PROOF_BUNDLE_PATH"
record_phase m045-s03-artifacts passed
record_phase m045-s03-bundle-shape started
assert_retained_bundle_shape \
  m045-s03-bundle-shape \
  "$BUNDLE_ROOT" \
  "$ARTIFACT_DIR/03-m045-s03-artifacts.txt" \
  "$LATEST_PROOF_BUNDLE_PATH"
record_phase m045-s03-bundle-shape passed

assert_file_contains_regex verifier-status "$PHASE_REPORT_PATH" '^m044-s04-replay\tpassed$' "M044 S04 failover rail did not pass" "$ARTIFACT_DIR/full-contract.log"
...
assert_file_contains_regex verifier-status "$PHASE_REPORT_PATH" '^m045-s02-runtime-completion\tpassed$' "M045 S02 runtime-completion rail did not pass" "$ARTIFACT_DIR/full-contract.log"
assert_file_contains_regex verifier-status "$PHASE_REPORT_PATH" '^m045-s03-e2e\tpassed$' "M045 S03 e2e rail did not pass" "$ARTIFACT_DIR/full-contract.log"
assert_file_contains_regex verifier-status "$PHASE_REPORT_PATH" '^m045-s03-artifacts\tpassed$' "M045 S03 artifacts were not retained" "$ARTIFACT_DIR/full-contract.log"
assert_file_contains_regex verifier-status "$PHASE_REPORT_PATH" '^m045-s03-bundle-shape\tpassed$' "M045 S03 bundle shape check did not pass" "$ARTIFACT_DIR/full-contract.log"

echo "verify-m045-s03: ok"
