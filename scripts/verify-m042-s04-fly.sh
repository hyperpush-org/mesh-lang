...
Read-only Fly verifier for the M042 packaged continuity/operator rail.
...
What it does not do:
  ...
  - no POST /work and no destructive owner-loss/recovery proof

Artifacts:
  .tmp/m042-s04/fly/

This script is a read-only sanity/config/log/probe rail. The authoritative
local continuity proof remains `bash scripts/verify-m042-s03.sh`; this lane only
inspects an already-deployed Fly app.
EOF
}
...
request_key = os.environ.get('CLUSTER_PROOF_REQUEST_KEY', '').strip()
...
if request_key and (len(request_key.strip()) == 0 or len(request_key) > 128):
    raise SystemExit('CLUSTER_PROOF_REQUEST_KEY must be 1..128 characters when provided')
if request_key and not re.fullmatch(r'[^\s].{0,127}', request_key):
    raise SystemExit('CLUSTER_PROOF_REQUEST_KEY must not start with whitespace')
...
summary = {
    ...
    'status_url': f'{normalized_base_url}/work/{request_key}' if request_key else '',
    ...
    ] + ([f'curl -fsS {normalized_base_url}/work/{request_key}'] if request_key else []),
}
...
import json
from pathlib import Path
print(json.loads(Path('.tmp/m042-s04/fly/input-validation.json').read_text())['app'])
PY
)"
  CLUSTER_PROOF_BASE_URL="$(python3 - <<'PY'
import json
from pathlib import Path
print(json.loads(Path('.tmp/m042-s04/fly/input-validation.json').read_text())['base_url'])
PY
)"
  CLUSTER_PROOF_REQUEST_KEY="$(python3 - <<'PY'
import json
from pathlib import Path
print(json.loads(Path('.tmp/m042-s04/fly/input-validation.json').read_text())['request_key'])
PY
)"
  export CLUSTER_PROOF_FLY_APP CLUSTER_PROOF_BASE_URL CLUSTER_PROOF_REQUEST_KEY
...
PY
  then
    m039_fail_phase "$phase" "Fly config drifted from the packaged operator contract" "$check_log" "$config_json_path"
  fi
}
...
print(json.dumps({'self': self_name, 'membership': membership, 'peers': peers}, indent=2))
PY
  then
    m039_fail_phase "$phase" "live /membership drifted from the packaged cluster contract" "$check_log" "$json_path"
  fi
}
...
assert_live_status_json() {
  ...
  if ! python3 - "$json_path" "$request_key" >"$check_log" 2>&1 <<'PY'
from pathlib import Path
import json
import re
import sys
...
if data['phase'] not in {'submitted', 'completed', 'rejected', 'missing', 'invalid'}:
    raise SystemExit(f"unexpected phase {data['phase']!r}")
if data['result'] not in {'pending', 'succeeded', 'rejected', 'missing', 'unknown'}:
    raise SystemExit(f"unexpected result {data['result']!r}")
for key in ('ingress_node', 'owner_node'):
    value = data[key]
    if value and '@' not in value:
        raise SystemExit(f'{key} must be empty or a node identity, found {value!r}')
for key in ('replica_node', 'execution_node'):
    value = data[key]
    if value and '@' not in value:
        raise SystemExit(f'{key} must be empty or a node identity, found {value!r}')

print(json.dumps({'request_key': data['request_key'], 'attempt_id': data['attempt_id'], 'phase': data['phase'], 'result': data['result']}, indent=2))
PY
  then
    m039_fail_phase "$phase" "live /work/:request_key drifted from the keyed status contract" "$check_log" "$json_path"
  fi
}
...
if ! m042_http_json_request membership-probe GET "$CLUSTER_PROOF_BASE_URL/membership" - 200 "$ARTIFACT_DIR/04-membership.http" "$ARTIFACT_DIR/04-membership.json" "live /membership"; then
...
assert_live_membership_json membership-probe "$ARTIFACT_DIR/04-membership.json"
...
if [[ -n "$CLUSTER_PROOF_REQUEST_KEY" ]]; then
  ...
  if ! m042_http_json_request keyed-status-probe GET "$CLUSTER_PROOF_BASE_URL/work/$CLUSTER_PROOF_REQUEST_KEY" - 200 "$ARTIFACT_DIR/05-keyed-status.http" "$ARTIFACT_DIR/05-keyed-status.json" "live /work/:request_key"; then
  ...
  assert_live_status_json keyed-status-probe "$ARTIFACT_DIR/05-keyed-status.json" "$CLUSTER_PROOF_REQUEST_KEY"
  m039_record_phase keyed-status-probe passed
fi

echo "verify-m042-s04-fly: ok"
