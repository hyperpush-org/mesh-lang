...
WEB_HTML_PATH="$ROOT_DIR/website/docs/.vitepress/dist/docs/web/index.html"
DATABASES_HTML_PATH="$ROOT_DIR/website/docs/.vitepress/dist/docs/databases/index.html"
TESTING_HTML_PATH="$ROOT_DIR/website/docs/.vitepress/dist/docs/testing/index.html"
CONCURRENCY_HTML_PATH="$ROOT_DIR/website/docs/.vitepress/dist/docs/concurrency/index.html"
...
pages = {
    ...
    'web': Path(sys.argv[4]),
    'databases': Path(sys.argv[5]),
    'testing': Path(sys.argv[6]),
    'concurrency': Path(sys.argv[7]),
    'distributed': Path(sys.argv[8]),
    'distributed_proof': Path(sys.argv[9]),
    'production_backend_proof': Path(sys.argv[10]),
}
...
checks = {
    ...
    'databases': {
        'contains': [
            ...
            'bash scripts/verify-m051-s02.sh',
            'retained compatibility wrappers',
            'repo-boundary maintained app/backend handoff after Production Backend Proof',
        ],
        'omits': [
            'https://github.com/snowdamiz/mesh-lang/blob/main/mesher/README.md',
            'reference-backend/README.md',
    ...
    'distributed': {
        'contains': [
            ...
            'Hyperpush product repo',
            'https://github.com/hyperpush-org/hyperpush-mono/blob/main/mesher/README.md',
            'mesh-lang keeps only the public proof-page wrappers and retained compatibility rails on this side of the boundary.',
        ],
        'omits': ['reference-backend/README.md'],
    },
    'distributed_proof': {
...
copy_file_or_fail retain-built-html "$WEB_HTML_PATH" "$BUILT_HTML_DIR/web.index.html" "missing built Web HTML snapshot after docs build"
copy_file_or_fail retain-built-html "$DATABASES_HTML_PATH" "$BUILT_HTML_DIR/databases.index.html" "missing built Databases HTML snapshot after docs build"
copy_file_or_fail retain-built-html "$TESTING_HTML_PATH" "$BUILT_HTML_DIR/testing.index.html" "missing built Testing HTML snapshot after docs build"
copy_file_or_fail retain-built-html "$CONCURRENCY_HTML_PATH" "$BUILT_HTML_DIR/concurrency.index.html" "missing built Concurrency HTML snapshot after docs build"
