...
- S02 is complete as a deliverable slice: the mixed-route section in `../hyperpush-mono/mesher/client/ROUTE-INVENTORY.md` is now expanded into fail-closed Issues, Alerts, and Settings tables with stable surface keys and normalized `live` / `mixed` / `shell-only` / `mock-only` classifications; the parser/test contract now rejects row drift at `routeSection/surfaceKey` granularity; the dev proof rail passes with explicit assertions for fine-grained mixed surfaces; and the issue seeding path is now self-contained inside the Playwright suites while the shared toast store leak in `../hyperpush-mono/mesher/client/hooks/use-toast.ts` is fixed.
- S03 is complete as a deliverable slice: `../hyperpush-mono/mesher/client/ROUTE-INVENTORY.md` now includes a fail-closed `## Backend gap map` with stable route/surface rows for mixed Issues, Alerts, and Settings surfaces plus the remaining mock-only Performance, Solana Programs, Releases, Bounties, and Treasury routes. Each row maps the current client promise to a concrete backend seam or missing seam and classifies support as `covered`, `missing-payload`, `missing-controls`, or `no-route-family`. The parser/test rail in `../hyperpush-mono/mesher/scripts/lib/client-route-inventory.mjs` and `../hyperpush-mono/mesher/scripts/tests/verify-client-route-inventory.test.mjs` now locks canonical section order, exact expected row sets, allowed support statuses, and blank/duplicate drift so backend maintainers can plan follow-up slices without re-auditing the client shell.
- The remaining milestone work is S04 canonical handoff/closeout, which now needs to package the completed inventory and backend gap map into the final maintainer-facing handoff and milestone-level validation rail.
...
- M061/S02 established the mixed-surface proof pattern: represent Issues, Alerts, and Settings truth as stable table rows keyed by durable surface ids, keep runtime `fallback` in diagnostics rather than as a contract classification, seed issue proof rows inside the browser suites through same-origin APIs so the rail stays self-contained, and treat `@/hooks/use-toast` as the authoritative shared toast store when debugging error-path console warnings
- M061/S03 established the backend-gap mapping pattern: keep backend-gap tables in the canonical route inventory, key them by stable `route/surface` identifiers, classify support with the fixed `covered` / `missing-payload` / `missing-controls` / `no-route-family` vocabulary, and fail closed on section order, duplicate rows, blank seam/work cells, or row-set drift without widening `readRouteInventory()` / `parseRouteInventoryMarkdown()` beyond top-level callers
