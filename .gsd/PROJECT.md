...
M060 is active, and S01-S02 are complete. The canonical `mesher/client` Issues shell now boots against the seeded default Mesher context through same-origin `/api/v1` reads, overlays live issue/list/stats/chart/detail/timeline data onto the existing shell, exposes truthful per-card live/derived/fallback summary markers, and routes supported maintainer actions (`Resolve`, `Reopen`, `Ignore`) through provider-owned same-origin mutations with destructive toast feedback on failures. The current closeout proof for the assembled Issues seam is the deterministic two-fixture seed helper plus passing dev/prod `issues live` Playwright runs.
...
- M060/S01-S02 established the first real maintainer loop in that canonical package: the Issues route reads the seeded default project through same-origin `/api/v1`, normalizes Mesher payloads through a typed live-overlay adapter, keeps unsupported shell fields on explicit fallback content, exposes per-card summary-source observability, and now drives Resolve/Reopen/Ignore through provider-owned mutation/refetch orchestration with visible toast feedback in dev and built-prod runs
...
- M060 is underway: the first two live Mesher-backed dashboard slices now prove same-origin browser transport, deterministic seeded read/action fixtures, explicit live/derived/fallback source observability, mounted toast feedback, and provider-owned issue mutation refresh without redesigning the shell or widening into new auth UX
...
- M060/S01-S02 established the live-shell wiring pattern for product-backed dashboard work: keep browser traffic on same-origin `/api/v1`, adapt Mesher payloads through a typed live/mock overlay instead of weakening the existing UI contract, keep mutation orchestration and post-write refetch inside `DashboardIssuesStateProvider`, expose source/state via stable `data-*` attributes and per-card summary markers for verification, and route backend read/write failures through the already-mounted toast surface instead of inventing a parallel error UX
...
- [ ] M060: Mesher Client Live Backend Wiring — connect the canonical `mesher/client` shell to the existing Mesher backend without redesigning the UI or deleting still-mocked surfaces
