...
#[test]
#[ignore = "manual failover proof removed with Continuity.promote(); replace with the S04 auto-only rail"]
fn e2e_m043_s02_failover_promotion_and_fenced_rejoin_keep_newer_truth() {
    build_cluster_proof();
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
...

        {
            let primary = spawned_primary_run1
                .as_mut()
                .expect("primary run1 missing before pending primary status");
            let standby = spawned_standby
                .as_mut()
        ...
        }
        {
            let primary = spawned_primary_run1
                .as_mut()
                .expect("primary run1 missing before pending standby status");
            let standby = spawned_standby
                .as_mut()
        ...
        }
        {
            let standby = spawned_standby
                .as_mut()
                .expect("standby process missing before promotion");
            let mut watched = [standby];
            wait_for_work_truth(
                &artifacts,
        ...
        let promote = json_body(
            &artifacts,
            "promote-standby",
            &post_json(standby_config.http_port, "/promote", "{}"),
            200,
            "standby promotion response",
        );
        assert!(required_bool(&promote, "ok"));
        assert_eq!(required_str(&promote, "cluster_role"), "primary");
        assert_eq!(required_u64(&promote, "promotion_epoch"), 1);
        assert_eq!(required_str(&promote, "replication_health"), "unavailable");
        assert_eq!(required_str(&promote, "error"), "");

        {
            let standby = spawned_standby
                .as_mut()
                .expect("standby process missing after promotion");
            let mut watched = [standby];
            wait_for_membership(
                &artifacts,
        ...
        }
        {
            let standby = spawned_standby
                .as_mut()
                .expect("standby process missing during promoted owner-lost status");
            let mut watched = [standby];
            wait_for_work_truth(
                &artifacts,
        ...
        let failover_retry = json_body(
            ...
            200,
            "same-key submit after promotion",
        );
        ...
        );
        {
            let standby = spawned_standby
                .as_mut()
                .expect("standby process missing during failover pending status");
            let mut watched = [standby];
            wait_for_work_truth(
                &artifacts,
                ...
                "pending promoted authority work",
            );
        }
        {
            let standby = spawned_standby
                .as_mut()
                .expect("standby process missing during failover completion");
            let mut watched = [standby];
            wait_for_work_truth(
                &artifacts,
                ...
                "completed promoted authority work",
            );
        }
...
{}",
    ...
    assert_log_contains(&primary_run1_logs, "continuity=runtime-native");
...
#[test]
fn continuity_api_manual_promotion_surface_is_disabled() {
...
#[test]
fn continuity_api_promote_wrong_arity_fails_at_compile_time() {
