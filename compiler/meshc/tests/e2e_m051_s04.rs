...
const WEB_PATH: &str = "website/docs/docs/web/index.md";
const DATABASES_PATH: &str = "website/docs/docs/databases/index.md";
const TESTING_PATH: &str = "website/docs/docs/testing/index.md";
const CONCURRENCY_PATH: &str = "website/docs/docs/concurrency/index.md";
...
const FIRST_CONTACT_CONTRACT_PATH: &str =
    "scripts/tests/verify-m050-s02-first-contact-contract.test.mjs";
const SECONDARY_SURFACES_CONTRACT_PATH: &str =
...
fn m051_s04_public_docs_scaffold_and_skill_stay_examples_first() {
    ...
    let web = read_and_archive(WEB_PATH, &artifacts);
    let databases = read_and_archive(DATABASES_PATH, &artifacts);
    let testing = read_and_archive(TESTING_PATH, &artifacts);
    let concurrency = read_and_archive(CONCURRENCY_PATH, &artifacts);
    ...
    assert_order(
        ...
        &[
            "- [Clustered Example](/docs/getting-started/clustered-example/)",
            "- [SQLite Todo starter](https://github.com/hyperpush-org/mesh-lang/blob/main/examples/todo-sqlite/README.md)",
            "- [PostgreSQL Todo starter](https://github.com/hyperpush-org/mesh-lang/blob/main/examples/todo-postgres/README.md)",
            "- [Production Backend Proof](/docs/production-backend-proof/)",
        ],
    );
    ...
    assert_contains_all(
        ...
        &databases,
        &[
            "- [Hyperpush product repo](https://github.com/hyperpush-org/hyperpush-mono) — repo-boundary maintained app/backend handoff after Production Backend Proof",
    ...
    for (path_label, source) in [
        ...
        (
            SECONDARY_SURFACES_CONTRACT_PATH,
            &secondary_surfaces_contract,
        ),
    ] {
    ...
    for (path_label, source) in [
        ...
        assert_contains(path_label, source, "Hyperpush product repo");
        assert_contains(path_label, source, "bash mesher/scripts/verify-maintainer-surface.sh");
    }
    assert_contains(
