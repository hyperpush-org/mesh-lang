...
const SHARED_COOKIE: &str = "mesh-m043-s01-cookie";
...
#[test]
fn e2e_m043_s01_missing_authority_fields_fail_closed() {
    let artifacts = artifact_dir("continuity-api-m043-malformed-contract");

    let membership = json!({
        "mode": "cluster",
        "self": "primary@127.0.0.1:4370",
        "membership": ["primary@127.0.0.1:4370", "standby@[::1]:4370"],
    ...
    let work = json!({
        ...
        "phase": "submitted",
        "result": "pending",
        "ingress_node": "primary@127.0.0.1:4370",
    ...
    assert!(
        ...
        "membership contract should fail closed"
    );
    ...
    assert!(work_result.is_err(), "work contract should fail closed");
}

#[test]
fn e2e_m043_s01_primary_submit_mirrors_truth_to_standby_without_promotion() {
    build_cluster_proof();

    let artifacts = artifact_dir("continuity-api-m043-primary-to-standby");
    ...
    let mut selected_attempt_id: Option<String> = None;
    let mut spawned_primary = Some(spawn_cluster_proof(primary_config.clone(), &artifacts));
    let mut spawned_standby = Some(spawn_cluster_proof(standby_config.clone(), &artifacts));

    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        {
            let primary = spawned_primary
                .as_mut()
                .expect("primary process missing before convergence");
        ...
        let submitted = {
            let primary = spawned_primary
                .as_mut()
                .expect("primary process missing before placement search");
            let standby = spawned_standby
                .as_mut()
        ...
        assert_eq!(submitted.status_code, 200, "primary submit must succeed");
        let attempt_id = required_str(&submitted.response, "attempt_id");
        selected_attempt_id = Some(attempt_id.clone());
        ...
        assert_work_truth(
            ...
            &ExpectedWorkTruth {
                ...
                attempt_id: selected_attempt_id.as_deref(),
                phase: "submitted",
                result: "pending",
        ...
        fs::copy(
            artifacts
                .join("primary-to-standby-search")
                .join("selected.http"),
...

        {
            let primary = spawned_primary
                .as_mut()
                .expect("primary process missing before pending primary status");
            let standby = spawned_standby
                .as_mut()
                .expect("standby process missing before pending primary status");
            let mut watched = [primary, standby];
            let pending_primary = wait_for_work_truth(
                &artifacts,
                "pending-primary",
                primary_config.http_port,
                &mut watched,
        ...
        }
        {
            let primary = spawned_primary
                .as_mut()
                .expect("primary process missing before pending standby status");
            let standby = spawned_standby
                .as_mut()
                .expect("standby process missing before pending standby status");
            let mut watched = [primary, standby];
            let pending_standby = wait_for_work_truth(
                &artifacts,
                "pending-standby",
                standby_config.http_port,
                &mut watched,
                ...
                "pending standby mirrored truth",
            );
            assert_work_truth(
                &pending_standby,
                &ExpectedWorkTruth {
        ...
        }
        {
            let primary = spawned_primary
                .as_mut()
                .expect("primary process missing before completed primary status");
            let standby = spawned_standby
                .as_mut()
                .expect("standby process missing before completed primary status");
            let mut watched = [primary, standby];
            let completed_primary = wait_for_work_truth(
                &artifacts,
                "completed-primary",
                primary_config.http_port,
                &mut watched,
        ...
        }
        {
            let primary = spawned_primary
                .as_mut()
                .expect("primary process missing before completed standby status");
            let standby = spawned_standby
                .as_mut()
                .expect("standby process missing before completed standby status");
            let mut watched = [primary, standby];
            let completed_standby = wait_for_work_truth(
                &artifacts,
                "completed-standby",
                standby_config.http_port,
                &mut watched,
                ...
                "completed standby mirrored truth",
            );
            assert_work_truth(
                &completed_standby,
                &ExpectedWorkTruth {
        ...
        let repeated_primary = json_body(
            &artifacts,
            "repeated-primary",
            &get_json(
                primary_config.http_port,
                &format!("/work/{}", selected_request_key.as_deref().unwrap()),
            ),
            200,
            "repeated primary keyed status",
        );
        assert_work_truth(
            &repeated_primary,
            &ExpectedWorkTruth {
        ...
        let repeated_standby = json_body(
            &artifacts,
            "repeated-standby",
            &get_json(
                standby_config.http_port,
    ...
    let primary_logs = stop_cluster_proof(
        spawned_primary
            .take()
    ...
    if let Err(payload) = result {
        panic!(
            "primary→standby mirrored truth assertions failed: {}\nartifacts: {}\nprimary stdout:\n{}\nprimary stderr:\n{}\nstandby stdout:\n{}\nstandby stderr:\n{}",
    ...
    let attempt_id = selected_attempt_id
        .as_deref()
        .expect("selected attempt id missing after successful run");

    assert_log_contains(&primary_logs, "continuity=runtime-native");
    assert_log_contains(&standby_logs, "continuity=runtime-native");
    assert_log_contains(
        &primary_logs,
        &format!(
            "[cluster-proof] keyed submit request_key={request_key} attempt_id={attempt_id} ingress={primary_node} owner={primary_node} replica={standby_node} replica_status=mirrored cluster_role=primary promotion_epoch=0 replication_health=healthy"
        ),
    );
    assert_log_contains(
        &primary_logs,
        &format!(
            "[mesh-rt continuity] transition=replica_ack request_key={request_key} attempt_id={attempt_id} owner={primary_node} replica={standby_node} cluster_role=primary promotion_epoch=0 replication_health=healthy replica_status=mirrored"
        ),
    );
    assert_log_contains(
        &primary_logs,
        &format!(
            "[cluster-proof] work executed request_key={request_key} attempt_id={attempt_id} execution={primary_node}"
        ),
    );
    assert_log_contains(
        &standby_logs,
        &format!(
            "[cluster-proof] keyed status request_key={request_key} attempt_id={attempt_id} phase=completed result=succeeded owner={primary_node} replica={standby_node} source={standby_node} replica_status=mirrored cluster_role=standby promotion_epoch=0 replication_health=healthy"
        ),
    );
    assert_log_absent(
        &standby_logs,
        &format!(
            "[cluster-proof] work executed request_key={request_key} attempt_id={attempt_id} execution={standby_node}"
        ),
    );
    assert_log_absent(&standby_logs, "transition=replication_degraded");
}
