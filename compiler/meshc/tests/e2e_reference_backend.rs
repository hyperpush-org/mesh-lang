...
fn assert_reference_backend_migration_succeeds(database_url: &str, command: &str) {
    ...
    assert_command_success(
        &output,
        &format!("meshc migrate scripts/fixtures/backend/reference-backend {command}"),
    );
}
...
fn e2e_reference_backend_deploy_artifact_smoke() {
    ...
    let test_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        let baseline_processed_jobs = smoke_health["worker"]["processed_jobs"]
            .as_i64()
            .unwrap_or(0);
        assert!(
...
fn e2e_reference_backend_worker_crash_recovers_job() {
    ...
    let test_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        assert_eq!(
            health_json["worker"]["last_recovery_count"].as_i64(),
            Some(1)
        );
        assert!(
...
fn e2e_reference_backend_worker_restart_is_visible_in_health() {
    ...
    let test_result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        let final_started_at = final_health["worker"]["started_at"]
            ...
            .to_string();
        assert!(!final_boot_id.is_empty(), "final boot_id must be non-empty");
        assert!(
